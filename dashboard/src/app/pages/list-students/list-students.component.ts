...
export class ListStudentsComponent {
  ...
  conflicts = computed(() =>
    this.studentStore.students().filter((s) => s.studentAssignments.some((a) => a.status === Status.Pending) && s.studentAssignments.length > 1)
  );

  // all where there is only one assignment and that one is pending
  singleAssignments = computed(() =>
    this.studentStore.students().filter((s) => s.studentAssignments.length === 1 && s.studentAssignments[0].status === Status.Pending)
  );

  // all where there is no assignment
  noAssignments = computed(() => this.studentStore.students().filter((s) => s.studentAssignments.length === 0));
